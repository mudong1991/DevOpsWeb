            ...
            <div class="unlock-input-con">
                <div class="unlock-input-overflow-con">
                    <div class="unlock-overflow-body" :style="{right: inputLeft}">
                        <input ref="inputEle" v-model="password" class="unlock-input" type="password" placeholder="密码同登录密码" autocomplete="off" />
...
<script>
  ...
  export default {
      ...
      methods: {
          ...
          handleUnlock () {
            baseService.checkUserPassword({password: this.password}, false, true).then(({data}) => {
              if (data.result_code === 1 || data.result_code === '1') {
                this.avatorLeft = '0px';
                this.inputLeft = '400px';
                this.password = '';
                this.$cookie.set('locking', '0');
                this.$emit('on-unlock');
              } else {
                MessageBox.alert('密码错误,请重新输入。如果忘了密码，清除浏览器缓存重新登录即可。');
              }
            });
          },
          unlockMousedown () {
